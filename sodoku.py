...
def generate_board(screen, user, cell_list):
        ...
        for x in cell_list:
            text = font.render(str(x.get_value()), 0, 'black')
            text_rect = text.get_rect(center=(( (x.get_col() + 1) * 66 ) / 2, ((x.get_row() + 1) * 66) / 2))
...
if __name__ == '__main__':
    ...
    if user == 1:
        game_board = sudoku_generator.generate_sudoku(9, 30)
        for i in range(0,9):
            for j in range(0,9):
                c = Cell(game_board[i][j], i, j, board_screen)
                cell_list.append(c)
    elif user == 2:
        game_board = sudoku_generator.generate_sudoku(9,40)
        for i in range(0,9):
            for j in range(0,9):
                c = Cell(game_board[i][j], i, j, board_screen)
                cell_list.append(c)
    elif user == 3:
        game_board = sudoku_generator.generate_sudoku(9, 50)
        for i in range(0,9):
            for j in range(0,9):
                c = Cell(game_board[i][j], i, j, board_screen)
                cell_list.append(c)
